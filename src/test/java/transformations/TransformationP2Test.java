...
public class TransformationP2Test {
    ...
    private void initializeTestCase() {
        transformation = new TransformationP2();
    ...
    private GraphModel startingGraphAfterTransformation() {
        ...
        final Vertex v7 = graph.insertVertex("V7", midpointCoords, nextLayerDescriptor).get();

        graph.insertEdge(v4, v5, nextLayerDescriptor);
        graph.insertEdge(v5, v7, nextLayerDescriptor);
        graph.insertEdge(v7, v6, nextLayerDescriptor);
        graph.insertEdge(v6, v4, nextLayerDescriptor);
        graph.insertEdge(v4, v7, nextLayerDescriptor);

        final InteriorNode i1 = graph.insertInterior("I1", nextLayerDescriptor, v4, v5, v7).get();
        final InteriorNode i2 = graph.insertInterior("I2", nextLayerDescriptor, v4, v6, v7).get();
        graph.insertEdge(i1, i);
        graph.insertEdge(i2, i);
    ...
    private GraphModel biggerGraphAfterTransformingI1() {
        ...
        Coordinates midpointCoords = PositionCalculator.getMidpointCoordinates(v2, v3);
        Coordinates v8Co = new Coordinates(midpointCoords.getX(), midpointCoords.getY(), midpointCoords.getZ());
        ...
        graph.insertEdge(v6, v8, nextLayer);
        graph.insertEdge(v8, v7, nextLayer);
        graph.insertEdge(v7, v5, nextLayer);
        graph.insertEdge(v5, v8, nextLayer);

        InteriorNode i3 = graph.insertInterior("I3", nextLayer, v5, v6, v8).get();
        InteriorNode i4 = graph.insertInterior("I4", nextLayer, v5, v8, v7).get();

        graph.insertEdge(i3, i1);
        graph.insertEdge(i4, i1);
        return graph;
    }
