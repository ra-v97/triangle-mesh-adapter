...
public class TransformationP4 implements Transformation {
    ...
    public boolean isApplicable(GraphModel graph, InteriorNode interior) {
        ...
        return isUpper(interior.getLabel()) &&
                ...
                graph.getEdgeBetweenNodes(iV[2], iV[0]).isPresent();
    }
    ...
    public void transform(GraphModel graph, InteriorNode interior) {
        ...
        LayerDescriptor nextLayerDescriptor =
                graph.resolveInteriorLayer(interior.getUUID()).get().getNextLayerDescriptor();
        Vertex[] iV = interior.getAdjacentVertices().toArray(new Vertex[0]);
        ...
        final Vertex v01 = graph.insertVertex("V4",
                new Coordinates(getCordsBetweenX(longest[0], longest[1]), getCordsBetweenY(longest[0], longest[1]),
                        longest[0].getZCoordinate()),
                nextLayerDescriptor
        ).get();
        final Vertex v02 = graph.insertVertex("V5",
                new Coordinates(getCordsBetweenX(longest[1], other), getCordsBetweenY(longest[1], other),
                        longest[1].getZCoordinate()),
                nextLayerDescriptor
        ).get();
        ...
        graph.insertEdge(v2, v0, nextLayerDescriptor);

        graph.insertEdge(v02, v01, nextLayerDescriptor);
        graph.insertEdge(v2, v01, nextLayerDescriptor);

        final InteriorNode i1 = graph.insertInterior("I", nextLayerDescriptor, v02, v1, v01).get();
        final InteriorNode i2 = graph.insertInterior("I", nextLayerDescriptor, v0, v01, v2).get();
        final InteriorNode i3 = graph.insertInterior("I", nextLayerDescriptor, v02, v01, v2).get();
